...
var WebSocketFileTransfer = new Class({
    ...
    start: function (outsocket) {
        ...
        if (!outsocket) {
            ...
            console.log('3');
            outsocket.messageHandler = function (event) {
                self.onMessage(event);
            }

            outsocket.errorHandler = function (event) {
                self.onError(event);
            }
            outsocket.closeHandler = function (event) {
                self.onClose();
            }
            if (this.options.type == 'binary') {
                outsocket.setBinaryType('blob');
            }
    ...
    onMessage: function (event) {
        ...
        var answer = response.params.code;

        if (response.id != MSG_FILE) {
            var error = {
        ...
        if (answer == 0) { // AUTH
            ...
            setTimeout(function () {
                ...
                var e = new WebSocketFileTransferEvent(self, self.file, /*curIndex + */ response.params.bytesreceived, self.file.size, self.startTime);
    ...
    initializeUpload: function () {
        ...
        var infos = {
            ...
            'code': 0, // 'AUTH: '
            'cat': this.options.uploadid,
            'stream': this.options.type,
            'auth': this.options.uploadtype,
        ...
        console.log('4');
        this.socket.send(JSON.encode(infos));
        infos = null;
    ...
    sendB64Slice: function (data) {
        this.socket.send('{"code": 2, "key":"' + this.key + '", "data":"' + window.btoa(data) + '"}');
        data = null;
    },
    /**
